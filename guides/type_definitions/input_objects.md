...
Input object types are complex inputs for GraphQL8 operations. They're great for fields that need a lot of structured input, like mutations or search fields. In a GraphQL8 request, it might look like this:
...
Like a Ruby `Hash`, an input object consists of keys and values. Unlike a Hash, its keys and value types must be defined statically, as part of the GraphQL8 system. For example, here's an input object, expressed in the [GraphQL8 Schema Definition Language](http://graphql.org/learn/schema/#type-language) (SDL):
...
Input object types extend {{ "GraphQL8::Schema::InputObject" | api_doc }} and define key-value pairs with the `argument(...)` method. For example:

```ruby
# app/graphql8/types/base_input_object.rb
# Add a base class
...
- calling `#[]` with the _camel-cased_ name of the argument (this is for compatibility with previous GraphQL8-Ruby versions)
...
- `@arguments`: A {{ "GraphQL8::Query::Arguments" | api_doc }} instance
- `@context`: The current {{ "GraphQL8::Query::Context" | api_doc }}
