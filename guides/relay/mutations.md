...
On the client-side, Relay also requires you to specify how it should interpret the response from your GraphQL8 server, which may require your server-side mutations to return payloads with specific fields.

## Mutation root

To add mutations to your GraphQL8 schema, define a mutation type and pass it to your schema:
...
Members of `MutationType` are _mutation fields_. For GraphQL8 in general, mutation fields are identical to query fields _except_ that they have side-effects (which mutate application state, eg, update the database).

For Relay-compliant GraphQL8, a mutation field must comply to a strict API. `GraphQL8::Relay` includes a mutation definition helper (see below) to make it simple.
...
To define a mutation, use `GraphQL8::Relay::Mutation.define`. Inside the block, you should configure:
...
```ruby
AddCommentMutation = GraphQL8::Relay::Mutation.define do
...
`graphql-ruby` uses your mutation to define some members of the schema. Under the hood, GraphQL8 creates:
...
```ruby
# Define a mutation:
AddCommentMutation = GraphQL8::Relay::Mutation.define { ... }
...
# => #<GraphQL8::Relay::Mutation @name="AddComment">
```
...
```ruby
CreateUser = GraphQL8::Relay::Mutation.define do
...
```ruby
MutationResult = GraphQL8::InterfaceType.define do
  name "MutationResult"
...
CreatePost = GraphQL8::Relay::Mutation.define do
  # ...
