...
module GraphQL8
  ...
  #   MediaUnion = GraphQL8::UnionType.define do
  #     name "Media"
  ...
  #  }
  #
  class UnionType < GraphQL8::BaseType
    ...
    # @return [Array<GraphQL8::ObjectType>] Types which may be found in this union
    def possible_types
      @clean_possible_types ||= begin
        if @dirty_possible_types.respond_to?(:map)
          @dirty_possible_types.map { |type| GraphQL8::BaseType.resolve_related_type(type) }
        else
          @dirty_possible_types
        end
      end
    ...
    # @return [GraphQL8::ObjectType, nil] The type named `type_name` if it exists and is a member of this {UnionType}, (else `nil`)
