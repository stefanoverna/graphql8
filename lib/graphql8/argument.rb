...
module GraphQL8
  ...
  #       type !types.String
  #       prepare ->(date) do
  #         return GraphQL8::ExecutionError.new("Invalid date format") unless DateValidator.valid?(date)
  ...
  class Argument
    ...
    def default_value=(new_default_value)
      if new_default_value == NO_DEFAULT_VALUE
        ...
        @default_value = GraphQL8::Argument.deep_stringify(new_default_value)
      end
    end

    # @!attribute name
    #   @return [String] The name of this argument on its {GraphQL8::Field} or {GraphQL8::InputObjectType}

    # @param new_input_type [GraphQL8::BaseType, Proc] Assign a new input type for this argument (if it's a proc, it will be called after schema initialization)
    def type=(new_input_type)
      @clean_type = nil
      @dirty_type = new_input_type
    end

    # @return [GraphQL8::BaseType] the input type for this argument
    def type
      @clean_type ||= GraphQL8::BaseType.resolve_related_type(@dirty_type)
    end
    ...
    def self.from_dsl(name, type_or_argument = nil, description = nil, default_value: NO_DEFAULT_VALUE, as: nil, prepare: DefaultPrepare, **kwargs, &block)
      ...
      if !type_or_argument.nil? && !type_or_argument.is_a?(GraphQL8::Argument)
        # Maybe a string, proc or BaseType
        kwargs[:type] = type_or_argument
      end

      if type_or_argument.is_a?(GraphQL8::Argument)
        type_or_argument.redefine(kwargs, &block)
      else
        GraphQL8::Argument.define(kwargs, &block)
      end
    end
