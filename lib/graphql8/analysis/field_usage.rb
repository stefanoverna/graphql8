...
module GraphQL8
  module Analysis
    ...
    #   Schema.query_analyzers << GraphQL8::Analysis::FieldUsage.new { |query, used_fields, used_deprecated_fields|
    #     puts "Used GraphQL8 fields: #{used_fields.join(', ')}"
    #     puts "Used deprecated GraphQL8 fields: #{used_deprecated_fields.join(', ')}"
    #   }
    #   Schema.execute(query_str)
    #   # Used GraphQL8 fields: Cheese.id, Cheese.fatContent, Query.cheese
    #   # Used deprecated GraphQL8 fields: Cheese.fatContent
    #
    class FieldUsage
      ...
      def call(memo, visit_type, irep_node)
        if irep_node.ast_node.is_a?(GraphQL8::Language::Nodes::Field) && visit_type == :leave
