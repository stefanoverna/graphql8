...
module GraphQL8
  module Analysis
    ...
    #   MySchema.query_analyzers << GraphQL8::Analysis::MaxQueryDepth.new(10)
    #
    class MaxQueryDepth < GraphQL8::Analysis::QueryDepth
      def initialize(max_depth)
        disallow_excessive_depth = ->(query, depth) {
          if depth > max_depth
            GraphQL8::AnalysisError.new("Query has depth of #{depth}, which exceeds max depth of #{max_depth}")
          else
            nil
          end
        }
