      ...
      def self.coerce(type, ast_node, variables)
        ...
        when Language::Nodes::VariableIdentifier
          ...
          when GraphQL8::EnumType
            ...
            v = ast_node.is_a?(GraphQL8::Language::Nodes::Enum) ? ast_node.name : ast_node
            if variables
      ...
      def self.from_arguments(ast_arguments, argument_owner, variables)
        ...
        argument_defns.each do |arg_name, arg_defn|
          ...
          if indexed_arguments.key?(arg_name)
            arg_value = ast_arg.is_a?(GraphQL8::Language::Nodes::Argument) ? ast_arg.value : ast_arg

            value_is_a_variable = arg_value.is_a?(GraphQL8::Language::Nodes::VariableIdentifier)
            ...
            if (!value_is_a_variable || (value_is_a_variable && variables.key?(arg_value.name)))
              ...
              if value.is_a?(GraphQL8::ExecutionError)
