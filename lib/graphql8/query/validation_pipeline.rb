...
module GraphQL8
  class Query
    ...
    class ValidationPipeline
      ...
      # @return [Array<GraphQL8::StaticValidation::Message>] Static validation errors for the query string
      def validation_errors
        ensure_has_validated
        @validation_errors
      end

      # @return [Hash<String, nil => GraphQL8::InternalRepresentation::Node] Operation name -> Irep node pairs
      ...
      def build_analyzers(schema, max_depth, max_complexity)
        if max_depth || max_complexity
          ...
          if max_depth
            qa << GraphQL8::Analysis::MaxQueryDepth.new(max_depth)
          end
          if max_complexity
            qa << GraphQL8::Analysis::MaxQueryComplexity.new(max_complexity)
          end
          qa
        else
