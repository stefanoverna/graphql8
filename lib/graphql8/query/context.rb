...
module GraphQL8
  class Query
    ...
    # It delegates `[]` to the hash that's passed to `GraphQL8::Query#initialize`.
    class Context
      module SharedMethods
        ...
        def skip
          GraphQL8::Execution::Execute::SKIP
        end
        ...
        # @param error [GraphQL8::ExecutionError] an execution error
        # @return [void]
        def add_error(error)
        ...
        # @example Print the GraphQL8 backtrace during field resolution
        #   puts ctx.backtrace
        #
        # @return [GraphQL8::Backtrace] The backtrace for this point in query execution
        def backtrace
          GraphQL8::Backtrace.new(self)
        end
      ...
      class ExecutionErrors
        ...
        def add(err_or_msg)
          err = case err_or_msg
          when String
            GraphQL8::ExecutionError.new(err_or_msg)
          when GraphQL8::ExecutionError
            err_or_msg
          else
            raise ArgumentError, "expected String or GraphQL8::ExecutionError, not #{err_or_msg.class} (#{err_or_msg.inspect})"
          end
      ...
      # `strategy` is required by GraphQL8::Batch
      alias_method :strategy, :execution_strategy

      def execution_strategy=(new_strategy)
        # GraphQL8::Batch re-assigns this value but it was previously not used
        # (ExecutionContext#strategy was used instead)
        # now it _is_ used, but it breaks GraphQL8::Batch tests
        @execution_strategy ||= new_strategy
      end

      # @return [GraphQL8::InternalRepresentation::Node] The internal representation for this query node
      def irep_node
        @irep_node ||= query.irep_selection
      end

      # @return [GraphQL8::Language::Nodes::Field] The AST node for the currently-executing field
      def ast_node
        @irep_node.ast_node
      end

      # @return [Array<GraphQL8::ExecutionError>] errors returned during execution
      attr_reader :errors

      # @return [GraphQL8::Query] The query whose context this is
      attr_reader :query

      # @return [GraphQL8::Schema]
      attr_reader :schema
      ...
      # @param query [GraphQL8::Query] the query who owns this context
      ...
      class FieldResolutionContext
        ...
        def inspect
          "#<GraphQL8 Context @ #{irep_node.owner_type.name}.#{field.name}>"
        end
        ...
        # @param new_value [Any] The GraphQL8-ready value
        # @api private
        def value=(new_value)
          case new_value
          when GraphQL8::Execution::Execute::PROPAGATE_NULL, nil
            @invalid_null = true
            @value = nil
            if @type.kind.non_null?
              @parent.received_null_child
            end
          when GraphQL8::Execution::Execute::SKIP
            @parent.skipped = true
            @parent.delete(self)
          else
            @value = new_value
        ...
        def received_null_child
          case @value
          when Hash
            self.value = GraphQL8::Execution::Execute::PROPAGATE_NULL
          when Array
        ...
        def list_of_non_null_items?(type)
          ...
          when GraphQL8::ListType
            type.of_type.is_a?(GraphQL8::NonNullType)
          else
...
GraphQL8::Schema::Context = GraphQL8::Query::Context
