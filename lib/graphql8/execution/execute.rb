  ...
  module Execution
    ...
    class Execute
      ...
      def execute(ast_operation, root_type, query)
        ...
        GraphQL8::Execution::Flatten.call(query.context)
      end
      ...
      module ExecutionFunctions
        ...
        def lazy_resolve_root_selection(result, query: nil, multiplex: nil)
          ...
          tracer.trace("execute_query_lazy", {multiplex: multiplex, query: query}) do
            GraphQL8::Execution::Lazy.resolve(result)
          end
        end
        ...
        def resolve_selection(object, current_type, current_ctx, mutation: false )
          ...
          selections_on_type.each do |name, child_irep_node|
            ...
            if mutation
              GraphQL8::Execution::Lazy.resolve(field_ctx)
            end
        ...
        def resolve_field(object, field_ctx)
          ...
          raw_value = begin
            ...
            rescue GraphQL8::UnauthorizedError => err
        ...
        def continue_resolve_field(raw_value, field_type, field_ctx)
          ...
          when Array
            if !field_type.list?
              ...
              list_errors = raw_value.each_with_index.select { |value, _| value.is_a?(GraphQL8::ExecutionError) }
              if list_errors.any?
        ...
        def resolve_value(value, field_type, field_ctx)
          ...
          elsif value.is_a?(GraphQL8::ExecutionError)
            if field_type.kind.non_null?
              PROPAGATE_NULL
            else
              nil
            end
          elsif value.is_a?(Array) && value.any? && value.all? {|v| v.is_a?(GraphQL8::ExecutionError)}
            if field_type.kind.non_null?
              PROPAGATE_NULL
            else
              nil
