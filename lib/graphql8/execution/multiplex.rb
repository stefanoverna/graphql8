...
module GraphQL8
  module Execution
    ...
    #
    # Validation errors and {GraphQL8::ExecutionError}s are handled in isolation:
    ...
    class Multiplex
      ...
      def initialize(schema:, queries:, context:)
        ...
        @tracers = schema.tracers + GraphQL8::Tracing.tracers + (context[:tracers] || [])
        # Support `context: {backtrace: true}`
        if context[:backtrace] && !@tracers.include?(GraphQL8::Backtrace::Tracer)
      ...
      class << self
        def run_all(schema, query_options, *args)
          queries = query_options.map { |opts| GraphQL8::Query.new(schema, nil, opts) }
          run_queries(schema, queries, *args)
        end

        # @param schema [GraphQL8::Schema]
        # @param queries [Array<GraphQL8::Query>]
        # @param context [Hash]
        ...
        def run_as_multiplex(multiplex)
          ...
          GraphQL8::Execution::Execute::ExecutionFunctions.lazy_resolve_root_selection(results, { multiplex: multiplex })
        ...
        def begin_query(query)
          ...
          if operation.nil? || !query.valid? || query.context.errors.any?
            ...
            begin
              GraphQL8::Execution::Execute::ExecutionFunctions.resolve_root_selection(query)
            rescue GraphQL8::ExecutionError => err
        ...
        # @param query [GraphQL8::Query] The query which was run
        ...
        def run_one_legacy(schema, query)
          query.result_values = if !query.valid?
            ...
            GraphQL8::Query::Executor.new(query).result
          end
        end

        def has_custom_strategy?(schema)
          schema.query_execution_strategy != GraphQL8::Execution::Execute ||
            schema.mutation_execution_strategy != GraphQL8::Execution::Execute ||
            schema.subscription_execution_strategy != GraphQL8::Execution::Execute
        end
        ...
        def instrument_and_analyze(multiplex, max_complexity:)
          GraphQL8::Execution::Instrumentation.apply_instrumenters(multiplex) do
            schema = multiplex.schema
            multiplex_analyzers = schema.multiplex_analyzers
            if max_complexity
              multiplex_analyzers += [GraphQL8::Analysis::MaxQueryComplexity.new(max_complexity)]
            end

            GraphQL8::Analysis.analyze_multiplex(multiplex, multiplex_analyzers)
            yield
          end
        end
      end
