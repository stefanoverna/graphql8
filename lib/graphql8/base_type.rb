...
module GraphQL8
  ...
  class BaseType
    include GraphQL8::Define::NonNullWithBang
    include GraphQL8::Define::InstanceDefinable
    include GraphQL8::Relay::TypeExtensions
    ...
    accepts_definitions :name, :description,
        ...
        :default_relay,
        {
          connection: GraphQL8::Define::AssignConnection,
          global_id_field: GraphQL8::Define::AssignGlobalIdField,
        }
    ...
    def name=(name)
      GraphQL8::NameValidator.validate!(name)
      @name = name
    end
    ...
    # @param other [GraphQL8::BaseType] compare to this object
    # @return [Boolean] are these types equivalent? (incl. non-null, list)
    # @see {ModifiesAnotherType#==} for override on List & NonNull types
    def ==(other)
      other.is_a?(GraphQL8::BaseType) && self.name == other.name
    end
    ...
    # @return [GraphQL8::NonNullType] a non-null version of this type
    def to_non_null_type
      GraphQL8::NonNullType.new(of_type: self)
    end

    # @return [GraphQL8::ListType] a list version of this type
    def to_list_type
      GraphQL8::ListType.new(of_type: self)
    end
    ...
    def valid_isolated_input?(value)
      valid_input?(value, GraphQL8::Query::NullContext)
    end

    def validate_isolated_input(value)
      validate_input(value, GraphQL8::Query::NullContext)
    end

    def coerce_isolated_input(value)
      coerce_input(value, GraphQL8::Query::NullContext)
    end

    def coerce_isolated_result(value)
      coerce_result(value, GraphQL8::Query::NullContext)
    end
    ...
    def validate_input(value, ctx = nil)
      ...
      if value.nil?
        GraphQL8::Query::InputValidationResult.new
      else
    ...
    # Return a GraphQL8 string for the type definition
    # @param schema [GraphQL8::Schema]
    # @param printer [GraphQL8::Schema::Printer]
    # @see {GraphQL8::Schema::Printer#initialize for additional options}
    # @return [String] type definition
    def to_definition(schema, printer: nil, **args)
      printer ||= GraphQL8::Schema::Printer.new(schema, **args)
