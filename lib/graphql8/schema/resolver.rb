...
module GraphQL8
  class Schema
    ...
    #
    # @see {GraphQL8::Schema::Mutation} for a concrete subclass of `Resolver`.
    # @see {GraphQL8::Function} `Resolver` is a replacement for `GraphQL8::Function`
    class Resolver
      include Schema::Member::GraphQL8TypeNames
      # Really we only need description from here, but:
      extend Schema::Member::BaseDSLMethods
      extend GraphQL8::Schema::Member::HasArguments
      ...
      #
      # If it returns a lazy object (like a promise), it will be synced by GraphQL8
      # (but the resulting value won't be used).
      #
      # @param args [Hash] The input arguments, if there are any
      # @raise [GraphQL8::ExecutionError] To add an error to the response
      # @raise [GraphQL8::UnauthorizedError] To signal an authorization failure
      ...
      def load_arguments(args)
        ...
        if prepare_lazies.any?
          GraphQL8::Execution::Lazy.all(prepare_lazies).then { prepared_args }
        else
          prepared_args
        end
      end
      ...
      class LoadApplicationObjectFailedError < GraphQL8::ExecutionError
        # @return [GraphQL8::Schema::Argument] the argument definition for the argument that was looked up
      ...
      #
      # @param type [Class, Module] A GraphQL8 type definition
      # @param id [String] A client-provided to look up
      # @param context [GraphQL8::Query::Context] the current context
              ...
              if (class_based_type = application_object_type.metadata[:type_class])
                context.schema.after_lazy(class_based_type.authorized?(application_object, context)) do |authed|
                  if authed
                    ...
                    raise GraphQL8::UnauthorizedError.new(
      ...
      class << self
        ...
        # @see {GraphQL8::Schema::Field#extras}
        ...
        def type(new_type = nil, null: nil)
          if new_type
            ...
            if @type_expr
              GraphQL8::Schema::Member::BuildType.parse_type(@type_expr, null: @null)
        ...
        # @see {GraphQL8::Schema::Argument#initialize} for the signature
        def argument(name, type, *rest, loads: nil, **kwargs, &block)
          ...
          if loads && arg_defn.type.list?
            ...
            def load_#{arg_defn.keyword}(values)
              GraphQL8::Execution::Lazy.all(values.map { |value| load_application_object(:#{arg_defn.keyword}, value) })
            end
            RUBY
          elsif loads
            class_eval <<-RUBY, __FILE__, __LINE__ + 1
