    ...
    class Member
      module Instrumentation
        ...
        def instrument(type, field)
          ...
          if (return_type.is_a?(GraphQL8::ObjectType) && return_type.metadata[:type_class]) ||
              return_type.is_a?(GraphQL8::InterfaceType) ||
              (return_type.is_a?(GraphQL8::UnionType) && return_type.possible_types.any? { |t| t.metadata[:type_class] })
        ...
        class ProxiedResolve
          ...
          def call(obj, args, ctx)
            ...
            if ctx.skip == result || ctx.schema.lazy?(result) || result.nil? || result.is_a?(GraphQL8::ExecutionError) || ctx.wrapped_object
              result
            else
