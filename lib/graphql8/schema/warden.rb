...
module GraphQL8
  class Schema
    # Restrict access to a {GraphQL8::Schema} with a user-defined filter.
    #
    ...
    class Warden
      ...
      # @return [Array<GraphQL8::BaseType>] Visible types in the schema
      def types
        @types ||= @schema.types.each_value.select { |t| visible_type?(t) }
      end

      # @return [GraphQL8::BaseType, nil] The type named `type_name`, if it exists (else `nil`)
      ...
      # @return [GraphQL8::Field, nil] The field named `field_name` on `parent_type`, if it exists
      ...
      # @return [Array<GraphQL8::BaseType>] The types which may be member of `type_defn`
      def possible_types(type_defn)
        @visible_possible_types ||= read_through { |type_defn| @schema.possible_types(type_defn).select { |t| visible_type?(t) } }
        @visible_possible_types[type_defn]
      end

      # @param type_defn [GraphQL8::ObjectType, GraphQL8::InterfaceType]
      # @return [Array<GraphQL8::Field>] Fields on `type_defn`
      def fields(type_defn)
        @visible_fields ||= read_through { |t| @schema.get_fields(t).each_value.select { |f| visible_field?(f) } }
        @visible_fields[type_defn]
      end

      # @param argument_owner [GraphQL8::Field, GraphQL8::InputObjectType]
      # @return [Array<GraphQL8::Argument>] Visible arguments on `argument_owner`
      def arguments(argument_owner)
        @visible_arguments ||= read_through { |o| o.arguments.each_value.select { |a| visible_field?(a) } }
        @visible_arguments[argument_owner]
      end

      # @return [Array<GraphQL8::EnumType::EnumValue>] Visible members of `enum_defn`
      ...
      # @return [Array<GraphQL8::InterfaceType>] Visible interfaces implemented by `obj_type`
