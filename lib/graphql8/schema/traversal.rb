...
module GraphQL8
  class Schema
    ...
    # @api private
    class Traversal
      # @return [Hash<String => GraphQL8::BaseType]
      attr_reader :type_map

      # @return [Hash<String => Hash<String => GraphQL8::Field>>]
      attr_reader :instrumented_field_map

      # @return [Hash<String => Array<GraphQL8::Field || GraphQL8::Argument || GraphQL8::Directive>]
      attr_reader :type_reference_map

      # @return [Hash<String => Array<GraphQL8::BaseType>]
      ...
      def initialize(schema, introspection: true)
        ...
        @field_instrumenters =
          schema.instrumenters[:field] +
            ...
            [
              GraphQL8::Relay::ConnectionInstrumentation,
              GraphQL8::Relay::EdgesInstrumentation,
              GraphQL8::Relay::Mutation::Instrumentation,
              GraphQL8::Schema::Member::Instrumentation,
            ] +
...
- Found __* types: #{@type_map.keys.select { |k| k.start_with?("__") }}
      ...
      def rewrap_resolved_type(late_bound_type, resolved_inner_type)
        ...
        when GraphQL8::Schema::LateBoundType
          resolved_inner_type
        else
      ...
      def visit(schema, member, context_description)
        ...
        when GraphQL8::Directive
          ...
          # Construct arguments class here, which is later used to generate GraphQL8::Query::Arguments
          # to be passed to a resolver proc
          GraphQL8::Query::Arguments.construct_arguments_class(member)
          ...
          if prev_type.nil?
            ...
            when GraphQL8::InputObjectType
              ...
              if type_defn.arguments_class.nil?
                GraphQL8::Query::Arguments.construct_arguments_class(type_defn)
              end
            end
        ...
        when Class
          if member.respond_to?(:graphql_definition)
            ...
            raise GraphQL8::Schema::InvalidTypeError.new("Unexpected traversal member: #{member} (#{member.class.name})")
          end
        else
          message = "Unexpected schema traversal member: #{member} (#{member.class.name})"
          raise GraphQL8::Schema::InvalidTypeError.new(message)
        end
      end
      ...
      def visit_field_on_type(schema, type_defn, field_defn, dynamic_field: false)
        ...
        if base_return_type.is_a?(GraphQL8::Schema::LateBoundType)
        ...
        # Construct arguments class here, which is later used to generate GraphQL8::Query::Arguments
        # to be passed to a resolver proc
        GraphQL8::Query::Arguments.construct_arguments_class(instrumented_field_defn)
      end

      def validate_type(member, context_description)
        error_message = GraphQL8::Schema::Validation.validate(member)
        if error_message
          raise GraphQL8::Schema::InvalidTypeError.new("#{context_description} is invalid: #{error_message}")
        end
      end
    end
  end
