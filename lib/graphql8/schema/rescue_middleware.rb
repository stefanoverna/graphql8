...
module GraphQL8
  class Schema
    ...
    # - If a handler is found, use it & return a {GraphQL8::ExecutionError}
    ...
    class RescueMiddleware
      ...
      # @yieldreturn [String] message to put in GraphQL8 response
      ...
      # Implement the requirement for {GraphQL8::Schema::MiddlewareChain}
      def call(*args)
        begin
          yield
      ...
      def attempt_rescue(err)
        rescue_table.each { |klass, handler|
          if klass.is_a?(Class) && err.is_a?(klass) && handler
            ...
            return GraphQL8::ExecutionError.new(message)
          end
        }
