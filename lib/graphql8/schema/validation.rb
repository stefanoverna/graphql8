...
module GraphQL8
  class Schema
    # This module provides a function for validating GraphQL8 types.
    #
    ...
    class Validation
      ...
      module Rules
        ...
        def self.assert_named_items_are_valid(item_name, get_items_proc)
          ->(type) {
            ...
            error_message = nil
            items.each do |item|
              item_message = GraphQL8::Schema::Validation.validate(item)
              if item_message
        ...
        ARGUMENTS_ARE_STRING_TO_ARGUMENT = Rules.assert_property_mapping(:arguments, String, GraphQL8::Argument)
        ...
        TYPE_IS_VALID_INPUT_TYPE = ->(type) {
          ...
          inner_type = outer_type.is_a?(GraphQL8::BaseType) ? outer_type.unwrap : nil

          case inner_type
          when GraphQL8::ScalarType, GraphQL8::InputObjectType, GraphQL8::EnumType
            # OK
          else
        ...
        SCHEMA_CAN_GENERATE_IDS = ->(schema) {
          has_id_field = schema.types.values.any? { |t| t.kind.fields? && t.all_fields.any? { |f| f.resolve_proc.is_a?(GraphQL8::Relay::GlobalIdResolve) } }
        ...
        RESERVED_TYPE_NAME = ->(type) {
          if type.name.start_with?('__') && !type.introspection?
            ...
            warn("Name #{type.name.inspect} must not begin with \"__\", which is reserved by GraphQL8 introspection.")
            nil
          else
            # ok name
          end
        ...
        RESERVED_NAME = ->(named_thing) {
          if named_thing.name.start_with?('__')
            # TODO: make this a hard failure in a later version
            warn("Name #{named_thing.name.inspect} must not begin with \"__\", which is reserved by GraphQL8 introspection.")
            nil
          else
            # no worries
          end
        ...
        INTERFACES_ARE_IMPLEMENTED = ->(obj_type) {
          ...
          obj_type.interfaces.each do |interface_type|
            interface_type.fields.each do |field_name, field_defn|
              ...
              if object_field.nil?
              ...
              elsif !GraphQL8::Execution::Typecast.subtype?(field_defn.type, object_field.type)
                ...
                object_field.arguments.each do |arg_name, arg_defn|
                  if field_defn.arguments[arg_name].nil? && arg_defn.type.is_a?(GraphQL8::NonNullType)
      ...
      RULES = {
        GraphQL8::Field => [
          ...
          Rules.assert_property(:type, GraphQL8::BaseType, GraphQL8::Schema::LateBoundType),
        ...
        GraphQL8::ObjectType => [
          Rules.assert_property_list_of(:interfaces, GraphQL8::InterfaceType),
          Rules::FIELDS_ARE_VALID,
          Rules::INTERFACES_ARE_IMPLEMENTED,
        ],
        GraphQL8::InputObjectType => [
          Rules::ARGUMENTS_ARE_STRING_TO_ARGUMENT,
          Rules::ARGUMENTS_ARE_VALID,
        ],
        GraphQL8::UnionType => [
          Rules.assert_property_list_of(:possible_types, GraphQL8::ObjectType),
          Rules::HAS_ONE_OR_MORE_POSSIBLE_TYPES,
        ],
        GraphQL8::InterfaceType => [
          Rules::FIELDS_ARE_VALID,
        ],
        GraphQL8::Schema => [
