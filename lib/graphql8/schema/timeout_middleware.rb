...
module GraphQL8
  class Schema
    ...
    #   MySchema.middleware << GraphQL8::Schema::TimeoutMiddleware.new(max_seconds: 2)
    #
    # @example Notifying Bugsnag on a timeout
    #   MySchema.middleware << GraphQL8::Schema::TimeoutMiddleware(max_seconds: 1.5) do |timeout_error, query|
    ...
    class TimeoutMiddleware
      ...
      # @return [GraphQL8::Schema::TimeoutMiddleware::TimeoutError] An error whose message will be added to the `errors` key
      def on_timeout(parent_type, parent_object, field_definition, field_args, field_context)
        err = GraphQL8::Schema::TimeoutMiddleware::TimeoutError.new(parent_type, field_definition)
        if @error_handler
      ...
      # This behaves like {GraphQL8::Query} but {#context} returns
      ...
      # Since it's a child of {GraphQL8::ExecutionError},
      # its message will be added to the response's `errors` key.
      #
      # To raise an error that will stop query resolution, use a custom block
      # to take this error and raise a new one which _doesn't_ descend from {GraphQL8::ExecutionError},
      # such as `RuntimeError`.
      class TimeoutError < GraphQL8::ExecutionError
