...
module GraphQL8
  class Schema
    # You can use the result of {GraphQL8::Introspection::INTROSPECTION_QUERY}
    ...
    module Loader
      ...
      # @deprecated Use {GraphQL8::Schema.from_introspection} instead
      ...
      class << self
        ...
        def define_type(type, type_resolver)
          ...
          when "ARGUMENT"
            ...
            if type["defaultValue"]
              kwargs[:default_value] = begin
                ...
                # Returns a `GraphQL8::Language::Nodes::Document`:
                dummy_query_ast = GraphQL8.parse(dummy_query_str)
          ...
          when "SCALAR"
            ...
            if GraphQL8::Schema::BUILT_IN_TYPES[type_name]
              GraphQL8::Schema::BUILT_IN_TYPES[type_name]
            else
              ScalarType.define(
                name: type["name"],
                description: type["description"],
