...
require "graphql8/query/arguments"
require "graphql8/query/arguments_cache"
require "graphql8/query/context"
require "graphql8/query/executor"
require "graphql8/query/literal_input"
require "graphql8/query/null_context"
require "graphql8/query/result"
require "graphql8/query/serial_execution"
require "graphql8/query/variables"
require "graphql8/query/input_validation_result"
require "graphql8/query/variable_validation_error"
require "graphql8/query/validation_pipeline"
...
module GraphQL8
  ...
  class Query
    ...
    class OperationNameMissingError < GraphQL8::ExecutionError
      def initialize(name)
        msg = if name.nil?
    ...
    # @param context [#[]] an arbitrary hash of values which you can access in {GraphQL8::Field#resolve}
    ...
    def initialize(schema, query_string = nil, query: nil, document: nil, context: nil, variables: nil, validate: true, subscription_topic: nil, operation_name: nil, root_value: nil, max_depth: nil, max_complexity: nil, except: nil, only: nil)
      ...
      @tracers = schema.tracers + GraphQL8::Tracing.tracers + (context ? context.fetch(:tracers, []) : [])
      # Support `ctx[:backtrace] = true` for wrapping backtraces
      if context && context[:backtrace] && !@tracers.include?(GraphQL8::Backtrace::Tracer)
        @tracers << GraphQL8::Backtrace::Tracer
      end
    ...
    # @return [Hash] A GraphQL8 response, with `"data"` and/or `"errors"` keys
    def result
      if !@executed
        with_prepared_ast {
    ...
    # @return [GraphQL8::Language::Nodes::OperationDefinition, nil]
    ...
    #
    # @return [GraphQL8::Query::Variables] Variables to apply to this query
    def variables
      @variables ||= begin
        with_prepared_ast {
    ...
    # @api private
    # @return [GraphQL8::Query::Arguments] Arguments for this node, merging default values, literal values and query variables
    ...
    # @param abstract_type [GraphQL8::UnionType, GraphQL8::InterfaceType]
    # @param value [Object] Any runtime value
    # @return [GraphQL8::ObjectType, nil] The runtime type of `value` from {Schema#resolve_type}
    ...
    def resolve_type(abstract_type, value = :__undefined__)
      ...
      if value.is_a?(GraphQL8::Schema::Object)
        value = value.object
      end
    ...
    def prepare_ast
      @prepared_ast = true
      @warden = GraphQL8::Schema::Warden.new(@filter, schema: @schema, context: @context)

      parse_error = nil
      @document ||= begin
        if query_string
          GraphQL8.parse(query_string, tracer: self)
        end
      rescue GraphQL8::ParseError => err
        parse_error = err
      ...
      if @document
        @document.definitions.each do |part|
          case part
          when GraphQL8::Language::Nodes::FragmentDefinition
            @fragments[part.name] = part
          when GraphQL8::Language::Nodes::OperationDefinition
        ...
        parse_error = GraphQL8::ExecutionError.new("No query string was present")
      ...
      if @operations.any?
        ...
        if @selected_operation.nil?
          operation_name_error = GraphQL8::Query::OperationNameMissingError.new(@operation_name)
        else
      ...
      @validation_pipeline = GraphQL8::Query::ValidationPipeline.new(
        query: self,
        validate: @validate,
