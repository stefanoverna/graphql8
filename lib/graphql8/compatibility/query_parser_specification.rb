...
require "graphql8/compatibility/query_parser_specification/query_assertions"
require "graphql8/compatibility/query_parser_specification/parse_error_specification"
      ...
      def self.build_suite(&block)
        Class.new(Minitest::Test) do
          ...
          def test_it_parses_the_introspection_query
            parse(GraphQL8::Introspection::INTROSPECTION_QUERY)
          end
          ...
          def test_it_parses_inputs
            ...
            assert_instance_of GraphQL8::Language::Nodes::Enum, inputs[4].value
            ...
            assert_instance_of GraphQL8::Language::Nodes::NullValue, inputs[9].value

            args = inputs[10].value.arguments
            assert_instance_of GraphQL8::Language::Nodes::NullValue, args.find{ |arg| arg.name == 'a' }.value
            assert_equal 'b', args.find{ |arg| arg.name == 'b' }.value

            values = inputs[11].value
            assert_equal 'a', values[0]
            assert_instance_of GraphQL8::Language::Nodes::NullValue, values[1]
          ...
          def test_it_doesnt_parse_nonsense_variables
            ...
            err_1 = assert_raises(GraphQL8::ParseError) do
              parse(query_string_1)
            end
            assert_equal [1,17], [err_1.line, err_1.col]

            err_2 = assert_raises(GraphQL8::ParseError) do
