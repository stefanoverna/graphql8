...
begin
  ...
  raise LoadError, "GraphQL8::Upgrader requires the 'parser' gem, please install it and/or add it to your Gemfile"
end
...
module GraphQL8
  module Upgrader
    ...
    class Transform
      # @param input_text [String] Untransformed GraphQL8-Ruby code
    ...
    # Turns `{X} = GraphQL8::{Y}Type.define do` into `class {X} < Types::Base{Y}`.
    class TypeDefineToClassTransform < Transform
      ...
      def initialize(base_class_pattern: "Types::Base\\3")
        @find_pattern = /( *)([a-zA-Z_0-9:]*) = GraphQL8::#{GRAPHQL_TYPES}Type\.define do/
        @replace_pattern = "\\1class \\2 < #{base_class_pattern}"
        @interface_replace_pattern = "\\1module \\2\n\\1  include #{base_class_pattern}"
      end

      def apply(input_text)
        if input_text.include?("GraphQL8::InterfaceType.define")
    ...
    # Turns `{X} = GraphQL8::Relay::Mutation.define do` into `class {X} < Mutations::BaseMutation`
    class MutationDefineToClassTransform < Transform
      ...
      def initialize(base_class_name: "Mutations::BaseMutation")
        @find_pattern = /([a-zA-Z_0-9:]*) = GraphQL8::Relay::Mutation.define do/
    ...
    class MutationResolveProcToMethodTransform < Transform
      ...
      def apply(input_text)
        if input_text =~ /GraphQL8::Relay::Mutation\.define/
    ...
    class Member
      ...
      def upgradeable?
        return false if @member.include? '< GraphQL8::Schema::'
