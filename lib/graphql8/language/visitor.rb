...
module GraphQL8
  module Language
    ...
    #   visitor[GraphQL8::Language::Nodes::Field] << ->(node) { total_field_count += 1 }
    #   # When we finish, print the field count:
    #   visitor[GraphQL8::Language::Nodes::Document].leave << ->(node) { p total_field_count }
    #   visitor.visit
    #   # => 6
    #
    class Visitor
      ...
      #   visitor[GraphQL8::Language::Nodes::Field] << ->(node, parent) { p "Here's a field" }
      def [](node_class)
      ...
      # Collect `enter` and `leave` hooks for classes in {GraphQL8::Language::Nodes}
      #
      # Access {NodeVisitor}s via {GraphQL8::Language::Visitor#[]}
      class NodeVisitor
