...
module GraphQL8
  module Language
    class Printer
      ...
      #    GraphQL8::Language::Printer.new.print(document)
      #    # => "{ ... }"
      #
      #
      # @example Building a custom printer
      #
      #  class MyPrinter < GraphQL8::Language::Printer
      ...
      def print_description(node, indent: "", first_in_block: true)
        ...
        description << GraphQL8::Language::BlockString.print(node.description, indent: indent)
      end
      ...
      def print_node(node, indent: "")
        ...
        when FalseClass, Float, Integer, NilClass, String, TrueClass, Symbol
          GraphQL8::Language.serialize(node)
        when Array
          "[#{node.map { |v| print_node(v) }.join(", ")}]".dup
        when Hash
          "{#{node.map { |k, v| "#{k}: #{print_node(v)}" }.join(", ")}}".dup
        else
          GraphQL8::Language.serialize(node.to_s)
        end
      end
