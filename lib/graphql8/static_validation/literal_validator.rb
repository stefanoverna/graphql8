  ...
  module StaticValidation
    ...
    class LiteralValidator
      ...
      def validate(ast_value, type)
        if type.nil?
        ...
        elsif ast_value.is_a?(GraphQL8::Language::Nodes::VariableIdentifier)
          true
          ...
          maybe_raise_if_invalid(ast_value) do
            if ast_value.is_a?(GraphQL8::Language::Nodes::Enum)
        ...
        elsif type.kind.input_object? && ast_value.is_a?(GraphQL8::Language::Nodes::InputObject)
          maybe_raise_if_invalid(ast_value) do
      ...
      # The GraphQL8 grammar supports variables embedded within scalars but graphql.js
      ...
      def constant_scalar?(ast_value)
        ...
        elsif ast_value.is_a?(GraphQL8::Language::Nodes::InputObject)
      ...
      def required_input_fields_are_present(type, ast_node)
        ...
        if @context.schema.error_bubbling
          ...
          missing_required_field_names.all? do |name|
            validate(GraphQL8::Language::Nodes::NullValue.new(name: name), @warden.arguments(type).find { |f| f.name == name }.type )
          end
        end
      end
