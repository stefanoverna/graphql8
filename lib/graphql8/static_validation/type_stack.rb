...
module GraphQL8
  module StaticValidation
    # - Ride along with `GraphQL8::Language::Visitor`
    ...
    class TypeStack
      ...
      TYPE_INFERRENCE_ROOTS = [
        GraphQL8::Language::Nodes::OperationDefinition,
        GraphQL8::Language::Nodes::FragmentDefinition,
      ]

      # @return [GraphQL8::Schema] the schema whose types are present in this document
      attr_reader :schema

      # When it enters an object (starting with query or mutation root), it's pushed on this stack.
      # When it exits, it's popped off.
      # @return [Array<GraphQL8::ObjectType, GraphQL8::Union, GraphQL8::Interface>]
      attr_reader :object_types

      # When it enters a field, it's pushed on this stack (useful for nested fields, args).
      # When it exits, it's popped off.
      # @return [Array<GraphQL8::Field>] fields which have been entered
      attr_reader :field_definitions

      # Directives are pushed on, then popped off while traversing the tree
      # @return [Array<GraphQL8::Node::Directive>] directives which have been entered
      attr_reader :directive_definitions

      # @return [Array<GraphQL8::Node::Argument>] arguments which have been entered
      attr_reader :argument_definitions

      # @return [Array<String>] fields which have been entered (by their AST name)
      attr_reader :path

      # @param schema [GraphQL8::Schema] the schema whose types to use when climbing this document
      # @param visitor [GraphQL8::Language::Visitor] a visitor to follow & watch the types
      ...
      PUSH_STRATEGIES = {
        GraphQL8::Language::Nodes::FragmentDefinition => FragmentDefinitionStrategy,
        GraphQL8::Language::Nodes::InlineFragment => InlineFragmentStrategy,
        GraphQL8::Language::Nodes::FragmentSpread => FragmentSpreadStrategy,
        GraphQL8::Language::Nodes::Argument => ArgumentStrategy,
        GraphQL8::Language::Nodes::Field => FieldStrategy,
        GraphQL8::Language::Nodes::Directive => DirectiveStrategy,
        GraphQL8::Language::Nodes::OperationDefinition => OperationDefinitionStrategy,
      }
