  ...
  module StaticValidation
    class VariableUsagesAreAllowed
      ...
      def validate(context)
        ...
        context.visitor[GraphQL8::Language::Nodes::Argument] << ->(node, parent) {
          node_values = if node.value.is_a?(Array)
            node.value
          else
            [node.value]
          end
          node_values = node_values.select { |value| value.is_a? GraphQL8::Language::Nodes::VariableIdentifier }
          ...
          when GraphQL8::Language::Nodes::InputObject
            ...
            if arg_type.is_a?(GraphQL8::InputObjectType)
      ...
      def validate_usage(arguments, arg_node, ast_var, context)
        ...
        if !ast_var.default_value.nil?
          unless var_type.is_a?(GraphQL8::NonNullType)
            ...
            var_type = GraphQL8::NonNullType.new(of_type: var_type)
          end
        end
