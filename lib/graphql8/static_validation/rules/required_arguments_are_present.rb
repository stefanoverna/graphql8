  ...
  module StaticValidation
    class RequiredArgumentsArePresent
      ...
      def validate(context)
        v = context.visitor
        v[GraphQL8::Language::Nodes::Field] << ->(node, parent) { validate_field(node, context) }
        v[GraphQL8::Language::Nodes::Directive] << ->(node, parent) { validate_directive(node, context) }
      end
