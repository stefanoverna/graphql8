  ...
  module StaticValidation
    ...
    class VariablesAreUsedAndDefined
      ...
      def validate(context)
        ...
        context.visitor[GraphQL8::Language::Nodes::OperationDefinition] << push_variable_context_stack
        ...
        context.visitor[GraphQL8::Language::Nodes::OperationDefinition].leave << pop_variable_context_stack

        context.visitor[GraphQL8::Language::Nodes::FragmentDefinition] << push_variable_context_stack
        context.visitor[GraphQL8::Language::Nodes::FragmentDefinition].leave << pop_variable_context_stack

        # For FragmentSpreads:
        #  - find the context on the stack
        #  - mark the context as containing this spread
        context.visitor[GraphQL8::Language::Nodes::FragmentSpread] << ->(node, parent) {
        ...
        #  - assign its AST node
        context.visitor[GraphQL8::Language::Nodes::VariableIdentifier] << ->(node, parent) {
        ...
        context.visitor[GraphQL8::Language::Nodes::Document].leave << ->(node, parent) {
          fragment_definitions = variable_usages_for_context.select { |key, value| key.is_a?(GraphQL8::Language::Nodes::FragmentDefinition) }
          operation_definitions = variable_usages_for_context.select { |key, value| key.is_a?(GraphQL8::Language::Nodes::OperationDefinition) }
