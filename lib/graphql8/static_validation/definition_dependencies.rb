  ...
  module StaticValidation
    ...
    class DefinitionDependencies
      ...
      def mount(context)
        ...
        visitor[GraphQL8::Language::Nodes::Document] << ->(node, prev_node) {
        ...
        visitor[GraphQL8::Language::Nodes::OperationDefinition] << ->(node, prev_node) {
          @node_paths[node] = NodeWithPath.new(node, context.path)
          current_parent = node
        }

        visitor[GraphQL8::Language::Nodes::OperationDefinition].leave << ->(node, prev_node) {
          current_parent = nil
        }

        visitor[GraphQL8::Language::Nodes::FragmentDefinition] << ->(node, prev_node) {
          @node_paths[node] = NodeWithPath.new(node, context.path)
          current_parent = node
        }

        visitor[GraphQL8::Language::Nodes::FragmentDefinition].leave << ->(node, prev_node) {
          current_parent = nil
        }

        visitor[GraphQL8::Language::Nodes::FragmentSpread] << ->(node, prev_node) {
          @node_paths[node] = NodeWithPath.new(node, context.path)
      ...
      class DependencyMap
        # @return [Array<GraphQL8::Language::Nodes::FragmentDefinition>]
        attr_reader :cyclical_definitions

        # @return [Hash<Node, Array<GraphQL8::Language::Nodes::FragmentSpread>>]
        ...
        # @return [Array<GraphQL8::Language::Nodes::AbstractNode>] dependencies for `definition_node`
        def [](definition_node)
        ...
        while fragment_node = independent_fragment_nodes.pop
          ...
          if fragment_usages.none?
            ...
            fragment_usages.each do |definition_node|
              ...
              if remaining.none? && definition_node.is_a?(GraphQL8::Language::Nodes::FragmentDefinition)
