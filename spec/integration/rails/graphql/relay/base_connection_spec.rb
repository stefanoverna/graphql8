...
describe GraphQL8::Relay::BaseConnection do
  module Encoder
    module_function
  ...
  describe ".connection_for_nodes" do
    it "resolves most specific connection type" do
      ...
      class SpecialArrayConnection < GraphQL8::Relay::BaseConnection; end
      GraphQL8::Relay::BaseConnection.register_connection_implementation(SpecialArray, SpecialArrayConnection)

      nodes = SpecialArray.new

      conn_class = GraphQL8::Relay::BaseConnection.connection_for_nodes(nodes)
  ...
  describe "arguments" do
    it "limits pagination args to positive numbers" do
      ...
      conn = GraphQL8::Relay::BaseConnection.new([], args, context: context)
  ...
  describe "#context" do
    it "Has public access to the field context" do
      conn = GraphQL8::Relay::BaseConnection.new([], {}, context: context)
  ...
  describe "#inspect" do
    it "inspects nicely" do
      ...
      conn = GraphQL8::Relay::BaseConnection.new([], args, context: context)
      assert_equal "#<GraphQL8::Relay::Connection @parent=nil @arguments={:first=>1, :last=>-1}>", conn.inspect
    end
  end
  ...
  describe "#encode / #decode" do
    ...
    it "defaults to base64" do
      conn = GraphQL8::Relay::BaseConnection.new([], {}, context: nil)
    ...
    it "raises an execution error when an invalid cursor is given" do
      ...
      assert_raises(GraphQL8::ExecutionError) do
        conn.decode("0")
      end
    end
  end
