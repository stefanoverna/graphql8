...
describe GraphQL8::Relay::ConnectionInstrumentation do
  it "replaces the previous field definition" do
    test_type = GraphQL8::ObjectType.define do
      name "Test"
  ...
  let(:build_schema) {
    ...
    test_field = GraphQL8::Field.define do
    ...
    [
      test_field,
      GraphQL8::Schema.define(query: test_type, raise_definition_error: true)
    ]
  }
  ...
  it "leaves the original field untouched" do
    ...
    assert_instance_of GraphQL8::Field::Resolve::MethodResolve, test_field.resolve_proc
    assert_instance_of GraphQL8::Relay::ConnectionResolve, conn_field.resolve_proc
  end
  ...
  it "passes connection behaviors to redefinitions" do
    ...
    assert_instance_of GraphQL8::Relay::ConnectionResolve, connection_field.resolve_proc
    assert_instance_of GraphQL8::Relay::ConnectionResolve, redefined_connection_field.resolve_proc
  end
  ...
  describe "after_built_ins instrumentation" do
    it "has access to connection objects" do
      ...
      assert_equal ["StarWars::FactionRecord", "Array", "GraphQL8::Relay::ArrayConnection"], ctx[:before_built_ins]
      # After the object is wrapped in a connection, it sees the connection object
      assert_equal ["StarWars::Faction", "StarWars::ShipConnectionWithParentType", "GraphQL8::Types::Relay::PageInfo"], ctx[:after_built_ins]
    end
  end
end
