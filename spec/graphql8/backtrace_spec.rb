...
describe GraphQL8::Backtrace do
  class LazyError
    def raise_err
      raise "Lazy Boom"
    end
  ...
  class ErrorAnalyzer
    def call(_memo, visit_type, irep_node)
      if irep_node.name == "raiseError"
        raise GraphQL8::AnalysisError, "this should not be wrapped by a backtrace, but instead, returned to the client"
      end
    end
  end
  ...
  let(:schema) {
    ...
    GRAPHQL
    GraphQL8::Schema.from_definition(defn, default_resolve: resolvers).redefine {
  ...
  let(:backtrace_schema) {
    schema.redefine(use: GraphQL8::Backtrace)
  }

  describe "GraphQL8 backtrace helpers" do
    it "raises a TracedError when enabled" do
      assert_raises(GraphQL8::Backtrace::TracedError) {
    ...
    it "annotates crashes from user code" do
      err = assert_raises(GraphQL8::Backtrace::TracedError) {
      ...
      # The message includes the GraphQL8 context
      rendered_table = [
      ...
      assert_includes err.message, "spec/graphql8/backtrace_spec.rb:42", "It includes the original backtrace"
    ...
    it "annotates errors from Query#result" do
      ...
      query = GraphQL8::Query.new(schema, query_str, context: context)
      err = assert_raises(GraphQL8::Backtrace::TracedError) {
        query.result
      }
      assert_instance_of RuntimeError, err.cause
    ...
    it "raises original exception instead of a TracedError when error does not occur during resolving" do
      ...
      assert_raises(RuntimeError) {
        instrumentation_schema.execute(GraphQL8::Introspection::INTROSPECTION_QUERY, context: { backtrace: true })
      }
    end
  end
