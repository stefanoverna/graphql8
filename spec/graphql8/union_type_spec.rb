...
describe GraphQL8::UnionType do
  let(:type_1) { OpenStruct.new(kind: GraphQL8::TypeKinds::OBJECT)}
  let(:type_2) { OpenStruct.new(kind: GraphQL8::TypeKinds::OBJECT)}
  let(:type_3) { OpenStruct.new(kind: GraphQL8::TypeKinds::SCALAR)}
  let(:union) {
    types = [type_1, type_2]
    GraphQL8::UnionType.define {
      name("MyUnion")
  ...
  it '#resolve_type raises error if resolved type is not in possible_types' do
    ...
    fake_ctx = OpenStruct.new(query: GraphQL8::Query.new(Dummy::Schema, ""))
