...
describe GraphQL8::Argument do
  it "is validated at schema build-time" do
    ...
    err = assert_raises(GraphQL8::Schema::InvalidTypeError) {
      schema = GraphQL8::Schema.define(query: query_type)
      schema.types
    }
  ...
  describe ".from_dsl" do
    it "accepts an existing argument" do
      ...
      arg = GraphQL8::Argument.from_dsl(:foo, existing)
    ...
    it "accepts a definition block after defining kwargs" do
      arg = GraphQL8::Argument.from_dsl(:foo, GraphQL8::STRING_TYPE) do
    ...
    it "accepts a definition block and yields the argument if the block has an arity of one" do
      arg = GraphQL8::Argument.from_dsl(:foo, GraphQL8::STRING_TYPE) do |argument|
    ...
    it "accepts a definition block with existing arg" do
      ...
      arg = GraphQL8::Argument.from_dsl(:foo, existing) do
    ...
    it "creates an argument from dsl arguments" do
      arg = GraphQL8::Argument.from_dsl(
        :foo,
        GraphQL8::STRING_TYPE,
        "A Description",
        default_value: "Bar"
      )
  ...
  it "accepts proc type" do
    argument = GraphQL8::Argument.define(name: :favoriteFood, type: -> { GraphQL8::STRING_TYPE })
    assert_equal GraphQL8::STRING_TYPE, argument.type
  end

  it "accepts a default_value" do
    argument = GraphQL8::Argument.define(name: :favoriteFood, type: GraphQL8::STRING_TYPE, default_value: 'Default')
    assert_equal 'Default', argument.default_value
    assert argument.default_value?
  end

  it "accepts a default_value of nil" do
    argument = GraphQL8::Argument.define(name: :favoriteFood, type: GraphQL8::STRING_TYPE, default_value: nil)
  ...
  describe "#as, #exposed_as" do
    it "accepts a `as` property to define the arg name at resolve time" do
      argument = GraphQL8::Argument.define(name: :favoriteFood, type: GraphQL8::STRING_TYPE, as: :favFood)
      assert_equal argument.as, :favFood
    end

    it "uses `name` or `as` for `expose_as`" do
      arg_1 = GraphQL8::Argument.define(name: :favoriteFood, type: GraphQL8::STRING_TYPE, as: :favFood)
      assert_equal arg_1.expose_as, "favFood"
      arg_2 = GraphQL8::Argument.define(name: :favoriteFood, type: GraphQL8::STRING_TYPE)
    ...
    it "can be set in the passed block" do
      argument = GraphQL8::Argument.define do
        name "arg"
        as "arg_name"
      end
  ...
  describe "prepare" do
    it "accepts a prepare proc and calls it to generate the prepared value" do
      ...
      argument = GraphQL8::Argument.define(name: :plusOne, type: GraphQL8::INT_TYPE, prepare: prepare_proc)
      assert_equal argument.prepare(1, {val: 1}), 2
    end

    it "returns the value itself if no prepare proc is provided" do
      argument = GraphQL8::Argument.define(name: :someNumber, type: GraphQL8::INT_TYPE)
