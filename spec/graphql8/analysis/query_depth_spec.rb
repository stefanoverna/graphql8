...
describe GraphQL8::Analysis::QueryDepth do
  let(:depths) { [] }
  let(:query_depth) { GraphQL8::Analysis::QueryDepth.new { |query, max_depth|  depths << query << max_depth } }
  let(:reduce_result) { GraphQL8::Analysis.analyze_query(query, [query_depth]) }
