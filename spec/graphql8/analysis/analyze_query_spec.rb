...
describe GraphQL8::Analysis do
  ...
  describe ".analyze_query" do
    ...
    let(:reduce_result) { GraphQL8::Analysis.analyze_query(query, analyzers) }
    let(:variables) { {} }
    let(:query) { GraphQL8::Query.new(Dummy::Schema, query_string, variables: variables) }
    let(:query_string) {%|
      {
        cheese(id: 1) {
          id
    ...
    describe "conditional analysis" do
      ...
      describe "when analyze? returns false" do
        let(:query) { GraphQL8::Query.new(Dummy::Schema, query_string, variables: variables, context: { analyze: false }) }
      ...
      describe "when analyze? returns true" do
        let(:query) { GraphQL8::Query.new(Dummy::Schema, query_string, variables: variables, context: { analyze: true }) }
    ...
    it "calls the defined analyzers" do
      ...
      expected_visited_types = [Dummy::DairyAppQuery.graphql_definition, Dummy::Cheese.graphql_definition, GraphQL8::INT_TYPE, GraphQL8::STRING_TYPE]
      assert_equal expected_visited_types, collected_types
      expected_node_counts = {
        GraphQL8::Language::Nodes::OperationDefinition => 1,
        GraphQL8::Language::Nodes::Field => 3,
      }
    ...
    describe "tracing" do
      ...
      it "emits traces" do
        ...
        assert_instance_of GraphQL8::Execution::Multiplex, multiplex_trace[:multiplex]

        assert_equal "analyze_query", query_trace[:key]
        assert_instance_of GraphQL8::Query, query_trace[:query]
      end
    end
    ...
    describe "when processing fields" do
      let(:connection_counter) {
        ->(memo, visit_type, irep_node) {
          memo ||= Hash.new { |h,k| h[k] = 0 }
          if visit_type == :enter
            if irep_node.ast_node.is_a?(GraphQL8::Language::Nodes::Field)
      ...
      let(:reduce_result) { GraphQL8::Analysis.analyze_query(query, analyzers) }
      let(:query) { GraphQL8::Query.new(StarWars::Schema, query_string, variables: variables) }
      let(:query_string) {%|
        query getBases {
          empire {
  ...
  describe ".visit_analyzers" do
      ...
      def call(memo, visit_type, irep_node)
        if visit_type == :enter
          if irep_node.ast_node.name == "id"
            raise GraphQL8::AnalysisError.new("Don't use the id field.", ast_node: irep_node.ast_node)
          end
        end
        memo
      end
      ...
      def call(memo, visit_type, irep_node)
        if visit_type == :enter
          if irep_node.ast_node.name == "flavor"
            memo[:errors] << GraphQL8::AnalysisError.new("Don't use the flavor field.", ast_node: irep_node.ast_node)
          end
        end
        memo
      end
    ...
    let(:reduce_result) { GraphQL8::Analysis.analyze_query(query, analyzers) }
    let(:query) { GraphQL8::Query.new(Dummy::Schema, query_string) }
    let(:query_string) {%|
      {
        cheese(id: 1) {
          id
