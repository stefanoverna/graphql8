...
describe GraphQL8::Analysis::FieldUsage do
  let(:result) { [] }
  let(:field_usage_analyzer) { GraphQL8::Analysis::FieldUsage.new { |query, used_fields, used_deprecated_fields| result << query << used_fields << used_deprecated_fields } }
  let(:reduce_result) { GraphQL8::Analysis.analyze_query(query, [field_usage_analyzer]) }
