...
describe GraphQL8::Analysis::QueryComplexity do
  let(:complexities) { [] }
  let(:query_complexity) { GraphQL8::Analysis::QueryComplexity.new { |this_query, complexity|  complexities << this_query << complexity } }
  let(:reduce_result) { GraphQL8::Analysis.analyze_query(query, [query_complexity]) }
  let(:variables) { {} }
  ...
  describe "relay types" do
    let(:query) { GraphQL8::Query.new(StarWars::Schema, query_string) }
    let(:query_string) {%|
    {
      rebels {
        ships {
  ...
  describe "custom complexities" do
    let(:query) { GraphQL8::Query.new(complexity_schema, query_string) }
    let(:complexity_schema) {
      complexity_interface = GraphQL8::InterfaceType.define do
        name "ComplexityInterface"
        field :value, types.Int
      end

      single_complexity_type = GraphQL8::ObjectType.define do
      ...
      double_complexity_type = GraphQL8::ObjectType.define do
