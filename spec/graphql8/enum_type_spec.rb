...
describe GraphQL8::EnumType do
  ...
  describe "invalid values" do
    it "rejects value names with a space" do
      assert_raises(GraphQL8::InvalidNameError) {
        InvalidEnumValueTest = GraphQL8::EnumType.define do
  ...
  describe "invalid name" do
    it "reject names with invalid format" do
      assert_raises(GraphQL8::InvalidNameError) do
        InvalidEnumNameTest = GraphQL8::EnumType.define do
  ...
  describe "values that are Arrays" do
    let(:schema) {
      enum = GraphQL8::EnumType.define do
        name "PluralEnum"
  ...
  it "accepts values array" do
    cow = GraphQL8::EnumType::EnumValue.define(name: "COW")
    goat = GraphQL8::EnumType::EnumValue.define(name: "GOAT")
    enum = GraphQL8::EnumType.define(name: "DairyAnimal", values: [cow, goat])
  ...
  describe "#dup" do
    it "copies the values map without altering the original" do
      enum_2 = enum.dup
      enum_2.add_value(GraphQL8::EnumType::EnumValue.define(name: "MUSKRAT"))
  ...
  describe "validates enum value name uniqueness" do
    it "raises an exception when adding a duplicate enum value name" do
      ...
      exception = assert_raises(RuntimeError) do
        enum.add_value(GraphQL8::EnumType::EnumValue.define(name: "COW"))
      end
