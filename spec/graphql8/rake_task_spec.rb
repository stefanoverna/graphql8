...
RakeTaskSchema = GraphQL8::Schema.from_definition(rake_task_schema_defn)

# Default task
GraphQL8::RakeTask.new(schema_name: "RakeTaskSchema")
# Configured task
GraphQL8::RakeTask.new(idl_outfile: "tmp/configured_schema.graphql") do |t|
  t.namespace = "graphql_custom"
  t.load_context = ->(task) { {filtered: true} }
  t.only = ->(member, ctx) { member.is_a?(GraphQL8::ScalarType) || (ctx[:filtered] && ["Query", "allowed"].include?(member.name)) }
  t.load_schema = ->(task) { RakeTaskSchema }
end

describe GraphQL8::RakeTask do
  describe "default settings" do
    ...
    it "writes JSON" do
      capture_io do
        Rake::Task["graphql8:schema:dump"].invoke
      end
      dumped_json = File.read("./schema.json")
      expected_json = JSON.pretty_generate(RakeTaskSchema.execute(GraphQL8::Introspection::INTROSPECTION_QUERY))
