...
describe GraphQL8::Schema::Printer do
  let(:schema) {
...
REASON
      value "WOZ", deprecation_reason: GraphQL8::Directive::DEFAULT_DEPRECATION_REASON
    end
    ...
    image_type = GraphQL8::ObjectType.define do
      name "Image"
    ...
    media_union_type = GraphQL8::UnionType.define do
      name "Media"
    ...
    create_post_mutation = GraphQL8::Relay::Mutation.define do
      name "CreatePost"
    ...
    subscription_root = GraphQL8::ObjectType.define do
      name "Subscription"
  ...
  describe ".print_introspection_schema" do
    it "returns the schema as a string for the introspection types" do
      # From https://github.com/graphql8/graphql-js/blob/6a0e00fe46951767287f2cc62e1a10b167b2eaa6/src/utilities/__tests__/schemaPrinter-test.js#L599
      expected = <<SCHEMA
schema {
  query: Root
}
...
"""
Marks an element of a GraphQL8 schema as no longer supported.
"""
...
"""
A Directive provides a way to describe alternate runtime execution and type validation behavior in a GraphQL8 document.
...
"""
A Directive can be adjacent to many parts of the GraphQL8 language, a
...
"""
A GraphQL8 Schema defines the capabilities of a GraphQL8 server. It exposes all
...
"""
The fundamental unit of any GraphQL8 Schema is the type. There are many kinds of
types in GraphQL8 as represented by the `__TypeKind` enum.
...
SCHEMA
      assert_equal expected.chomp, GraphQL8::Schema::Printer.print_introspection_schema
    end
  end
...
SCHEMA
      assert_match expected, GraphQL8::Schema::Printer.print_schema(custom_schema)
    end
...
SCHEMA
      ...
      assert_equal expected.chomp, GraphQL8::Schema::Printer.print_schema(schema)
    end

    it 'prints a schema without directives' do
      query_type = Class.new(GraphQL8::Schema::Object) do
        graphql_name 'Query'
      ...
      schema = Class.new(GraphQL8::Schema) do
        query query_type
      end

      expected = "type Query {\n  foobar: Int!\n}"
      assert_equal expected, GraphQL8::Schema::Printer.new(schema).print_schema
    end
  end
...
SCHEMA
      assert_equal expected.chomp, GraphQL8::Schema::Printer.new(schema).print_type(schema.types['Post'])
    end
...
SCHEMA
      assert_equal expected.chomp, GraphQL8::Schema::Printer.new(schema).print_type(schema.types['Sub'])
    end

    it "can print arguments that use non-standard Ruby objects as default values" do
      backing_object = Struct.new(:value)

      scalar_type = GraphQL8::ScalarType.define do
        name "SomeType"
...
SCHEMA
      ...
      assert_equal expected.chomp, GraphQL8::Schema::Printer.new(schema).print_type(query_root)
    end
  end
...
SCHEMA
      ...
      assert_equal expected.chomp, GraphQL8::Schema::Printer.new(schema, context: context, only: only_filter).print_schema
    end
  end
end
