...
describe GraphQL8::Schema::BuildFromDefinition do
  ...
  def build_schema_and_compare_output(definition)
    built_schema = GraphQL8::Schema.from_definition(definition)
    assert_equal definition, GraphQL8::Schema::Printer.print_schema(built_schema)
    built_schema
  end
...
type Hello {
  str: String
      ...
      refute built_schema.directives['skip'] == GraphQL8::Directive::SkipDirective
      refute built_schema.directives['include'] == GraphQL8::Directive::IncludeDirective
      refute built_schema.directives['deprecated'] == GraphQL8::Directive::DeprecatedDirective
    end
...
type Type implements Interface {
  field(argument: String): String
      ...
      schema = GraphQL8::Schema.from_definition(schema_definition)
...
SCHEMA
      err = assert_raises(GraphQL8::Schema::InvalidDocumentError) do
        GraphQL8::Schema.from_definition(schema)
      end
...
SCHEMA
  ...
  describe "executable schema with resolver maps" do
    ...
    let(:schema) { GraphQL8::Schema.from_definition(definition, default_resolve: resolvers) }
  ...
  describe "executable schemas from string" do
    ...
    it "calls methods with args if args are defined" do
      schema = GraphQL8::Schema.from_definition(schema_defn)
    ...
    describe "hash of resolvers with defaults" do
      ...
      let(:schema) { GraphQL8::Schema.from_definition(schema_defn, default_resolve: resolve_hash) }
      let(:resolve_hash) {
        h = base_hash
        h["Query"] ||= {}
    ...
    describe "custom resolve behavior" do
      ...
      it "accepts a default_resolve callable" do
        schema = GraphQL8::Schema.from_definition(schema_defn, default_resolve: AppResolver.new)
    ...
    describe "custom parser behavior" do
      ...
      it 'accepts a parser callable' do
        assert_raises(BadParser::ParseError) do
          GraphQL8::Schema.from_definition(schema_defn, parser: BadParser)
        end
      end
    end
  end
