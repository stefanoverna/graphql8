...
describe GraphQL8::Schema::Validation do
  ...
  def assert_validation_warns(object, warning)
    assert_output("", warning + "\n") { GraphQL8::Schema::Validation.validate(object) }
  end

  describe "validating Fields" do
    let(:unnamed_field) {
      GraphQL8::Field.define do
    ...
    it "cannot use reserved name" do
      assert_validation_warns invalid_name_field, 'Name "__Something" must not begin with "__", which is reserved by GraphQL8 introspection.'
    end

    it "requires a BaseType for type" do
      assert_error_includes untyped_field, "must return GraphQL8::BaseType, not Symbol"
    end

    it "requires String => Argument arguments" do
      assert_error_includes bad_arguments_field, "must map String => GraphQL8::Argument, not Symbol => Symbol"
    end
  ...
  describe "validating BaseType" do
    ...
    it "cannot use reserved name" do
      assert_validation_warns invalid_name_type, 'Name "__Something" must not begin with "__", which is reserved by GraphQL8 introspection.'
    end
  ...
  describe "validating ObjectTypes" do
    ...
    it "requires an Array for interfaces" do
      assert_error_includes invalid_interface_member_object, "must contain GraphQL8::InterfaceType, not Symbol"
    end

    it "validates the fields" do
      assert_error_includes invalid_field_object, "must return GraphQL8::BaseType, not Symbol"
    end

    it "requires interfaces to be implemented" do
      iface_1 = GraphQL8::InterfaceType.define do
        name "Interface1"
        field :field_1, types.String
        field :field_2, types.Int
      end

      iface_2 = GraphQL8::InterfaceType.define do
        name "Interface2"
      ...
      iface_3 = GraphQL8::InterfaceType.define do
        name "Interface3"
      ...
      obj_type = GraphQL8::ObjectType.define do
        name "Object1"
  ...
  describe "validating UnionTypes" do
    ...
    it "requires an array of ObjectTypes for possible_types" do
      assert_error_includes non_array_union, "must be an Array of GraphQL8::ObjectType, not a #{integer_class_name}"

      assert_error_includes non_object_type_union, "must contain GraphQL8::ObjectType, not GraphQL8::InterfaceType"
    end
  ...
  describe "validating InputObjectTypes" do
    let(:invalid_arguments_input) {
      input = GraphQL8::InputObjectType.define do
    ...
    let(:invalid_argument_member_input) {
      GraphQL8::InputObjectType.define do
    ...
    it "requires {String => Argument} arguments" do
      assert_error_includes invalid_arguments_input, "map String => GraphQL8::Argument, not #{integer_class_name} => Symbol"
    end
  ...
  describe "validating Arguments" do
    ...
    it "cannot use reserved name" do
      assert_validation_warns invalid_name_argument, 'Name "__Something" must not begin with "__", which is reserved by GraphQL8 introspection.'
    end

    it "allows null default value for nullable argument" do
      assert_nil GraphQL8::Schema::Validation.validate(null_default_value)
    end
  end
