...
describe GraphQL8::Schema::TimeoutMiddleware do
  let(:max_seconds) { 1 }
  let(:timeout_middleware) {  GraphQL8::Schema::TimeoutMiddleware.new(max_seconds: max_seconds) }
  let(:timeout_schema) {
    ...
    nested_sleep_type = GraphQL8::ObjectType.define do
      name "NestedSleep"
  ...
  describe "with a custom block" do
    let(:timeout_middleware) {
      GraphQL8::Schema::TimeoutMiddleware.new(max_seconds: max_seconds) do |err, query|
