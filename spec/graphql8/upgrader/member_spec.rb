...
require "spec_helper"
require './lib/graphql8/upgrader/member.rb'

describe GraphQL8::Upgrader::Member do
  def upgrade(old)
    GraphQL8::Upgrader::Member.new(old).upgrade
  end
  ...
  describe 'name' do
    it 'removes the name field if it can be inferred from the class' do
      old = %{
        UserType = GraphQL8::ObjectType.define do
          name "User"
        end
      }
      new = %{
    ...
    it 'removes the name field if it can be inferred from the class and under a module' do
      old = %{
        Types::UserType = GraphQL8::ObjectType.define do
          name "User"
        end
      }
      new = %{
    ...
    it 'upgrades the name into graphql_name if it can\'t be inferred from the class' do
      old = %{
        TeamType = GraphQL8::ObjectType.define do
          name "User"
        end
      }
      new = %{
      ...
      old = %{
        UserInterface = GraphQL8::InterfaceType.define do
          name "User"
        end
      }
      new = %{
      ...
      old = %{
        UserEnum = GraphQL8::EnumType.define do
          name "User"
        end
      }
      new = %{
  ...
  describe 'definition' do
    it 'upgrades the .define into class based definition' do
      old = %{UserType = GraphQL8::ObjectType.define do
      end}
...
RUBY
      ...
      old = %{UserUnion = GraphQL8::UnionType.define do
      end}
      new = %{class UserUnion < Types::BaseUnion
      end}
      assert_equal new, upgrade(old)

      old = %{UserEnum = GraphQL8::EnumType.define do
      end}
      new = %{class UserEnum < Types::BaseEnum
      end}
      assert_equal new, upgrade(old)

      old = %{UserInput = GraphQL8::InputObjectType.define do
      end}
      new = %{class UserInput < Types::BaseInputObject
      end}
      assert_equal new, upgrade(old)

      old = %{UserScalar = GraphQL8::ScalarType.define do
      end}
    ...
    it 'upgrades including the module' do
      old = %{Module::UserType = GraphQL8::ObjectType.define do
      end}
  ...
  describe "fixtures" do
    class ActiveRecordTypeToClassTransform < GraphQL8::Upgrader::Transform
      def initialize
    ...
    class InputObjectsToInputsTransform < GraphQL8::Upgrader::Transform
      def apply(input_text)
    ...
    def custom_upgrade(original_text)
      ...
      type_transforms = GraphQL8::Upgrader::Member::DEFAULT_TYPE_TRANSFORMS.map { |t|
        if t == GraphQL8::Upgrader::TypeDefineToClassTransform
          GraphQL8::Upgrader::TypeDefineToClassTransform.new(base_class_pattern: "Platform::\\3s::Base")
        else
          t
        end
      }
      ...
      field_transforms = GraphQL8::Upgrader::Member::DEFAULT_FIELD_TRANSFORMS.dup
      field_transforms.unshift(GraphQL8::Upgrader::ConfigurationToKwargTransform.new(kwarg: "visibility"))
      upgrader = GraphQL8::Upgrader::Member.new(original_text, type_transforms: type_transforms, field_transforms: field_transforms)
      upgrader.upgrade
    end
