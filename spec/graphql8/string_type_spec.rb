...
describe GraphQL8::STRING_TYPE do
  let(:string_type) { GraphQL8::STRING_TYPE }
  ...
  describe "coerce_result" do
    ...
    describe "encoding" do
      ...
      describe "when the result is not UTF-8 and cannot be transcoded" do
        ...
        it "raises GraphQL8::StringEncodingError" do
          err = assert_raises(GraphQL8::StringEncodingError) { subject }
          assert_equal "String \"#{string}\" was encoded as ASCII-8BIT! GraphQL8 requires an encoding compatible with UTF-8.", err.message
    ...
    describe "when the schema defines a custom handler" do
      let(:schema) {
        GraphQL8::Schema.define do
          query(GraphQL8::ObjectType.define(name: "Query"))
          type_error ->(err, ctx) {
            ctx.errors << err
            "🌾"
          }
      ...
      it "calls the handler" do
        ...
        assert_instance_of GraphQL8::StringEncodingError, err
      end
    end
  end
