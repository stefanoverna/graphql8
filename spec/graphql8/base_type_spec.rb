...
describe GraphQL8::BaseType do
  it "becomes non-null with !" do
    type = GraphQL8::EnumType.new
    non_null_type = !type
    assert_equal(GraphQL8::TypeKinds::NON_NULL, non_null_type.kind)
    assert_equal(type, non_null_type.of_type)
    assert_equal(GraphQL8::TypeKinds::NON_NULL, (!GraphQL8::STRING_TYPE).kind)
  end

  it "can be compared" do
    obj_type = Dummy::Milk.graphql_definition
    assert_equal(!GraphQL8::INT_TYPE, !GraphQL8::INT_TYPE)
    refute_equal(!GraphQL8::FLOAT_TYPE, GraphQL8::FLOAT_TYPE)
    assert_equal(
      GraphQL8::ListType.new(of_type: obj_type),
      GraphQL8::ListType.new(of_type: obj_type)
    )
    refute_equal(
      GraphQL8::ListType.new(of_type: obj_type),
  ...
  describe "#name" do
    describe "when containing spaces" do
      BaseNameSpaceTest = GraphQL8::BaseType.define do
        name "Some Invalid Name"
      end

      it "is invalid" do
        assert_raises(GraphQL8::InvalidNameError) { BaseNameSpaceTest.name }
      end
    end

    describe "when containing colons" do
      BaseNameColonsTest = GraphQL8::BaseType.define do
        name "Some::Invalid::Name"
      end

      it 'is invalid' do
        assert_raises(GraphQL8::InvalidNameError) { BaseNameColonsTest.name }
      end
    end
  end
