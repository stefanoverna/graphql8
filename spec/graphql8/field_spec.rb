...
FieldSpecReturnType = GraphQL8::ObjectType.define do
  name "FieldReturn"
  field :id, types.Int
  field :source, types.String, hash_key: :source
end

describe GraphQL8::Field do
  ...
  it "accepts arguments definition" do
    number = GraphQL8::Argument.define(name: :number, type: -> { GraphQL8::INT_TYPE })
    field = GraphQL8::Field.define(type: FieldSpecReturnType, arguments: [number])
  ...
  describe ".property " do
    ...
    it "has nil property if not defined" do
      no_prop_field = GraphQL8::Field.define { }
  ...
  describe "#name" do
    it "must be a string" do
      dummy_query = GraphQL8::ObjectType.define do
        name "QueryType"
      end

      invalid_field = GraphQL8::Field.new
      ...
      err = assert_raises(GraphQL8::Schema::InvalidTypeError) {
        GraphQL8::Schema.define(query: dummy_query, raise_definition_error: true)
      }
  ...
  describe "reusing a GraphQL8::Field" do
    let(:schema) {
      int_field = GraphQL8::Field.define do
        type types.Int
  ...
  describe "#redefine" do
    ...
    it "keeps the same resolve_proc when it is not a NameResolve" do
      int_field = GraphQL8::Field.define do
        name "a"
        resolve ->(obj, _, _) { 'GraphQL8 is Kool' }
      end
  ...
  describe "#resolve_proc" do
    it "ensures the definition was called" do
      ...
      field_with_proc = GraphQL8::Field.define do
        resolve ->(o, a, c) { :whatever }
      end

      field_with_class = GraphQL8::Field.define do
