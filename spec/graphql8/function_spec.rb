...
describe GraphQL8::Function do
  class TestFunc < GraphQL8::Function
    argument :name, GraphQL8::STRING_TYPE
  ...
  it "has default values" do
    default_func = GraphQL8::Function.new
  ...
  describe "use in a schema" do
    let(:schema) {
      ...
      relay_mutation = GraphQL8::Relay::Mutation.define do
        name "Test"
        function TestFunc.new
      end

      mutation_type = GraphQL8::ObjectType.define do
        name "Mutation"
    ...
    it "can be used as a field" do
      query_str = <<-GRAPHQL
      { test(name: "graphql8") { name }}
      GRAPHQL
      res = schema.execute(query_str)
      assert_equal "graphql8", res["data"]["test"]["name"]
    end

    it "can be used as a mutation" do
      query_str = <<-GRAPHQL
      mutation { test(input: {clientMutationId: "123", name: "graphql8"}) { name, clientMutationId } }
      GRAPHQL
      res = schema.execute(query_str)
      assert_equal "graphql8", res["data"]["test"]["name"]
    end
  ...
  describe "when overriding" do
    ...
    it "can add to arguments" do
      ...
      assert_equal GraphQL8::STRING_TYPE, field.type
      assert_equal ["name", "age"], field.arguments.keys
    end
  end
end
