...
describe GraphQL8::InternalRepresentation::Rewrite do
  let(:schema) {
    GraphQL8::Schema.from_definition <<-GRAPHQL
    type Query {
      plant(id: ID!): Plant
      fruit(id: ID!): [Fruit!]
    }
  ...
  let(:validator) { GraphQL8::StaticValidation::Validator.new(schema: schema) }
  let(:query) { GraphQL8::Query.new(schema, query_string) }
  let(:rewrite_result) {
  ...
  describe "fragment definition without query" do
    ...
    let(:validator) {
      rules = GraphQL8::StaticValidation::ALL_RULES - [
        GraphQL8::StaticValidation::FragmentsAreUsed
      ]
      GraphQL8::StaticValidation::Validator.new(schema: schema, rules: rules)
    }
