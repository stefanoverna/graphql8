...
describe GraphQL8::Query do
  ...
  let(:document) { GraphQL8.parse(query_string) }

  let(:query) { GraphQL8::Query.new(
    schema,
    query_string,
  ...
  describe "#operation_name" do
    ...
    describe "assigning operation_name=" do
      ...
      it "runs the assigned name" do
        query = GraphQL8::Query.new(Dummy::Schema, query_string, operation_name: "q3")
  ...
  it "uses root_value as the object for the root type" do
    result = GraphQL8::Query.new(schema, '{ root }', root_value: "I am root").result
    assert_equal 'I am root', result.fetch('data').fetch('root')
  end

  it "exposes fragments" do
    assert_equal(GraphQL8::Language::Nodes::FragmentDefinition, query.fragments["cheeseFields"].class)
  end
  ...
  describe "parse errors" do
    ...
    it "can be configured to raise" do
      ...
      assert_raises(GraphQL8::ParseError) {
  ...
  describe "#mutation?" do
    ...
    it "returns true if the selected operation is a mutation" do
      query_query = GraphQL8::Query.new(schema, query_string, operation_name: "Q")
      assert_equal false, query_query.mutation?
      assert_equal true, query_query.query?

      mutation_query = GraphQL8::Query.new(schema, query_string, operation_name: "M")
  ...
  describe "validate: false" do
    it "doesn't validate the query" do
      ...
      query = GraphQL8::Query.new(schema, invalid_query_string)
      query.validate = false
      assert_equal true, query.valid?
      assert_equal 0, query.static_errors.length

      # Can pass keyword argument
      query = GraphQL8::Query.new(schema, invalid_query_string, validate: false)
      assert_equal true, query.valid?
      assert_equal 0, query.static_errors.length

      # Can pass `true`
      query = GraphQL8::Query.new(schema, invalid_query_string, validate: true)
  ...
  describe "validating with optional arguments and variables: nil" do
    it "works" do
      ...
      GRAPHQL
      query = GraphQL8::Query.new(schema, query_str, variables: nil)
      assert query.valid?
    end
  end
  ...
  describe 'NullValue type arguments' do
    ...
    let(:schema) { GraphQL8::Schema.from_definition(schema_definition, default_resolve: default_resolver) }
  ...
  describe '#internal_representation' do
    it "includes all definition roots" do
      assert_kind_of GraphQL8::InternalRepresentation::Node, query.internal_representation.operation_definitions["getFlavor"]
      assert_kind_of GraphQL8::InternalRepresentation::Node, query.internal_representation.fragment_definitions["cheeseFields"]
      assert_kind_of GraphQL8::InternalRepresentation::Node, query.internal_representation.fragment_definitions["edibleFields"]
      assert_kind_of GraphQL8::InternalRepresentation::Node, query.internal_representation.fragment_definitions["milkFields"]
      assert_kind_of GraphQL8::InternalRepresentation::Node, query.internal_representation.fragment_definitions["dairyFields"]
    end
  end

  describe '#irep_selection' do
    it "returns the irep for the selected operation" do
      assert_kind_of GraphQL8::InternalRepresentation::Node, query.irep_selection
      assert_equal 'getFlavor', query.irep_selection.name
    end

    it "returns nil when there is no selected operation" do
      query = GraphQL8::Query.new(schema, '# Only a comment')
