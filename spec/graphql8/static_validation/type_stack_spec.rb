...
class TypeCheckValidator
  ...
  def validate(context)
    ...
    context.visitor[GraphQL8::Language::Nodes::Field] << ->(node, parent) {
      self.class.checks << context.object_types.map {|t| t.name || t.kind.name }
    }
  end
end

describe GraphQL8::StaticValidation::TypeStack do
  let(:query_string) {%|
    query getCheese {
  ...
  let(:validator) { GraphQL8::StaticValidation::Validator.new(schema: Dummy::Schema, rules: [TypeCheckValidator]) }
