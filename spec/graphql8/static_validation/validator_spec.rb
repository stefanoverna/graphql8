...
describe GraphQL8::StaticValidation::Validator do
  let(:validator) { GraphQL8::StaticValidation::Validator.new(schema: Dummy::Schema) }
  ...
  describe "tracing" do
    ...
    let(:query) { GraphQL8::Query.new(Dummy::Schema, query_string, context: {tracers: [TestTracing]}) }
    ...
    it "emits a trace" do
      ...
      assert_instance_of GraphQL8::Query, validate_trace[:query]
  ...
  describe "error format" do
    ...
    let(:document) { GraphQL8.parse_with_racc(query_string, filename: "not_a_real.graphql") }
    let(:query) { GraphQL8::Query.new(Dummy::Schema, nil, document: document) }
