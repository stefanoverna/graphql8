...
describe GraphQL8::Authorization do
  module AuthTest
    class Box
      attr_reader :value
    ...
    class IntegerObjectEdge < GraphQL8::Types::Relay::BaseEdge
      node_type(IntegerObject)
    end

    class IntegerObjectConnection < GraphQL8::Types::Relay::BaseConnection
    ...
    class DoHiddenStuff < GraphQL8::Schema::RelayClassicMutation
      def self.visible?(ctx)
        super && (ctx[:hidden_mutation] ? false : true)
      end
    end

    class DoHiddenStuff2 < GraphQL8::Schema::Mutation
      def self.visible?(ctx)
        super && !ctx[:hidden_mutation]
      end
    end

    class DoInaccessibleStuff < GraphQL8::Schema::RelayClassicMutation
      def self.accessible?(ctx)
        super && (ctx[:inaccessible_mutation] ? false : true)
      end
    end

    class DoUnauthorizedStuff < GraphQL8::Schema::RelayClassicMutation
      def self.authorized?(obj, ctx)
    ...
    class Schema < GraphQL8::Schema
      ...
      def self.unauthorized_object(err)
        if err.object.respond_to?(:replacement)
          ...
          raise GraphQL8::ExecutionError, "Unauthorized #{err.type.graphql_name}: #{err.object}"
        end
      end
  ...
  describe "applying the visible? method" do
    ...
    it "refuses to resolve to hidden enum values" do
      assert_raises(GraphQL8::EnumType::UnresolvedValueError) do
