...
describe GraphQL8::Query::SerialExecution::ValueResolution do
  let(:schema) {
    day_of_week_enum = GraphQL8::EnumType.define do
      name "DayOfWeek"
    ...
    some_object = GraphQL8::ObjectType.define do
      name "SomeObject"
      interfaces [interface]
    end

    other_object = GraphQL8::ObjectType.define do
      name "OtherObject"
    end
  ...
  describe "interface type resolution" do
    describe "when type can't be resolved" do
      ...
      it "raises an error" do
        err = assert_raises(GraphQL8::UnresolvedTypeError) { result }
