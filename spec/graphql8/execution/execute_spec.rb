...
ExecuteSuite = GraphQL8::Compatibility::ExecutionSpecification.build_suite(GraphQL8::Execution::Execute)
LazyExecuteSuite = GraphQL8::Compatibility::LazyExecutionSpecification.build_suite(GraphQL8::Execution::Execute)

describe GraphQL8::Execution::Execute do
  describe "null propagation on mutation root" do
    ...
    describe "when root fields are non-nullable" do
      let(:schema) { GraphQL8::Schema.from_definition <<-GRAPHQL
        type Mutation {
          push(int: Int!): Int!
        }
  ...
  describe "when a member of a list of non-null type returns nil" do
    let(:schema) {
      node_type = GraphQL8::ObjectType.define do
        name "Node"
  ...
  describe "tracing" do
    if defined?(Rails)
      it "emits traces" do
        ...
        assert_instance_of GraphQL8::Query, query_eager[:query]
        ...
        assert_instance_of GraphQL8::Query, query_lazy[:query]

        assert_instance_of GraphQL8::Execution::Multiplex, multiplex[:multiplex]
      end
    end
  end
end
