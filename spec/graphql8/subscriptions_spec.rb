...
class InMemoryBackend
  class Subscriptions < GraphQL8::Subscriptions
...
class ClassBasedInMemoryBackend < InMemoryBackend
  class Payload < GraphQL8::Schema::Object
    field :str, String, null: false
    field :int, Integer, null: false
  end

  class PayloadType < GraphQL8::Schema::Enum
  ...
  class StreamInput < GraphQL8::Schema::InputObject
    argument :user_id, ID, required: true
    argument :type, PayloadType, required: false, default_value: "ONE"
  end

  class Subscription < GraphQL8::Schema::Object
    ...
    field :failed_event, Payload, null: false, resolve: ->(o, a, c) { raise GraphQL8::ExecutionError.new("unauthorized") }  do
      argument :id, ID, required: true
    end
  end
...
class FromDefinitionInMemoryBackend < InMemoryBackend
  ...
  Resolvers = {
    "Subscription" => {
      ...
      "myEvent" => ->(o,a,c) { o },
      "event" => ->(o,a,c) { o },
      "failedEvent" => ->(o,a,c) { raise GraphQL8::ExecutionError.new("unauthorized") },
    },
  }
  Schema = GraphQL8::Schema.from_definition(SchemaDefinition, default_resolve: Resolvers).redefine do
...
describe GraphQL8::Subscriptions do
  before do
    ...
    describe "using #{in_memory_backend_class}" do
      ...
      describe "pushing updates" do
        ...
        subscription ($id: ID!){
          ...
          # Let's see what GraphQL8 sent over the wire:
      ...
      describe "trigger" do
        let(:error_payload_class) {
          Class.new {
            ...
            def str
              raise GraphQL8::ExecutionError.new("This is handled")
            end
          }
        }
      ...
      describe ".trigger" do
        it "raises when event name is not found" do
          err = assert_raises(GraphQL8::Subscriptions::InvalidTriggerError) do
