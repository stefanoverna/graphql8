...
describe GraphQL8::ObjectType do
  let(:type) { Dummy::Cheese.graphql_definition }

  it "doesn't allow double non-null constraints" do
    assert_raises(GraphQL8::DoubleNonNullTypeError) {
      DoubleNullObject = GraphQL8::ObjectType.define do
        name "DoubleNull"
  ...
  it "doesn't allow invalid name" do
    exception = assert_raises(GraphQL8::InvalidNameError) {
      InvalidNameObject = GraphQL8::ObjectType.define do
  ...
  describe "interfaces" do
    ...
    it "doesnt convolute field names that differ with underscore" do
      interface = Module.new do
        ...
        field :id, GraphQL8::ID_TYPE, null: false
      end

      object = Class.new(GraphQL8::Schema::Object) do
        graphql_name 'TestObject'
        implements interface
        global_id_field :id

        # When the validation for `id` is run for `_id`, it will fail because
        # GraphQL8::STRING_TYPE cannot be transformed into a GraphQL8::ID_TYPE
        field :_id, String, description: 'database id', null: true
      end

      assert_equal nil, GraphQL8::Schema::Validation.validate(object.to_graphql)
    end
  end

  it "accepts fields definition" do
    last_produced_dairy = GraphQL8::Field.define(name: :last_produced_dairy, type: Dummy::DairyProduct)
    cow_type = GraphQL8::ObjectType.define(name: "Cow", fields: [last_produced_dairy])
  ...
  describe "#implements" do
    ...
    it "can be used to inherit fields from the interface" do
      type_1 = GraphQL8::ObjectType.define do
        name 'Hello'
        implements Dummy::Edible
        implements Dummy::AnimalProduct
      end

      type_2 = GraphQL8::ObjectType.define do
        name 'Hello'
        implements Dummy::Edible
        implements Dummy::AnimalProduct, inherit: true
      end

      type_3 = GraphQL8::ObjectType.define do
        name 'Hello'
  ...
  describe '#get_field' do
    it "exposes fields" do
      ...
      assert_equal(GraphQL8::TypeKinds::NON_NULL, field.type.kind)
      assert_equal(GraphQL8::TypeKinds::SCALAR, field.type.of_type.kind)
    end
  ...
  describe "#dup" do
    it "copies fields and interfaces without altering the original" do
      ...
      type_2.fields["nonsense"] = GraphQL8::Field.define(name: "nonsense", type: type)
