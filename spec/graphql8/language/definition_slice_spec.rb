...
describe GraphQL8::Language::DefinitionSlice do
  let(:document) { GraphQL8::Language::Parser.parse(query_string) }
  ...
  describe "document with multiple queries but no subdependencies" do
    ...
    it "returns just the query definition" do
      assert_equal GraphQL8::Language::Nodes::Document.new(definitions: [document.definitions[0]]).to_query_string,
        document.slice_definition("getVersion").to_query_string
      assert_equal GraphQL8::Language::Nodes::Document.new(definitions: [document.definitions[1]]).to_query_string,
