...
describe GraphQL8::Language::DocumentFromSchemaDefinition do
  let(:subject) { GraphQL8::Language::DocumentFromSchemaDefinition }
  ...
  describe "#document" do
    ...
    let(:schema) { GraphQL8::Schema.from_definition(schema_idl) }

    let(:expected_document) { GraphQL8.parse(expected_idl) }
    ...
    describe "with an except filter" do
      ...
      let(:document) {
        subject.new(
          schema,
          except: ->(m, _ctx) { m.is_a?(GraphQL8::BaseType) && m.name == "Type" }
        ).document
      }
    ...
    describe "with an only filter" do
      ...
      let(:document) {
        subject.new(
          schema,
          only: ->(m, _ctx) { !(m.is_a?(GraphQL8::ScalarType) && m.name == "CustomScalar") }
        ).document
      }
    ...
    describe "when printing excluding only introspection types" do
      let(:expected_idl) { <<-GRAPHQL
        ...
        # used by GraphQL8 to represent free-form human-readable text.
        scalar String
