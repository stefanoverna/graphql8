...
describe GraphQL8::Language::Parser do
  subject { GraphQL8::Language::Parser }
  ...
  describe "anonymous fragment extension" do
    ...
    it "creates an anonymous fragment definition" do
      assert fragment.is_a?(GraphQL8::Language::Nodes::FragmentDefinition)
  ...
  it "parses the test schema" do
    schema = Dummy::Schema
    schema_string = GraphQL8::Schema::Printer.print_schema(schema)
  ...
  describe ".parse_file" do
    it "assigns filename to all nodes" do
      ...
      doc = GraphQL8.parse_file(example_filename)
    ...
    it "raises errors with filename" do
      ...
      err = assert_raises(GraphQL8::ParseError) {
        GraphQL8.parse_file(error_filename)
      }

      assert_includes err.message, error_filename

      error_filename_2 = "spec/support/parser/filename_example_error_2.graphql"
      err_2 = assert_raises(GraphQL8::ParseError) {
        GraphQL8.parse_file(error_filename_2)
      }
  ...
  it "serves traces" do
    TestTracing.clear
    GraphQL8.parse("{ t: __typename }", tracer: TestTracing)
    ...
    assert_instance_of GraphQL8::Language::Nodes::Document, parse_trace[:result]
  end
end
